...
table.on('sort(pageTable)',function(obj){_this.where['sort[name]']=obj.field;_this.where['sort[dir]']=obj.type==='asc'?'a':'d';dataTable.reloadData();});table.on('tool(pageTable)',function(obj){var event=obj.event,data=obj.data;switch(event){case'setup':break;case'queue':_this.showQueue(data);break;}});//绘制日期控件
...
element.on('tab(runStatus)',function(){var typ=$(this).data('type');if(typ!==that.where.status){that.where.status=typ;dataTable.reloadData();}});};_proto.showQueue=function showQueue(task){//admin.openDialog('','','')
console.log(task);};return TablePage;}();var taskTable=new TablePage();taskTable.init('#pageTable',pageData.table.cols,pageData.table.data);});{/literal}</script>