...
dropdown.render({elem:'#newMsgButton',data:pageData.newMsgItems,click:function click(obj){that.openEditor(obj.id,obj.title);}});//用户选择
var userXmlSelect=xmSelect.render({name:'uid',el:'#userXmlSelect',height:'240px',radio:true,empty:'无用户',paging:true,// 开启分页
pageSize:10,// 每页条数
data:[],initValue:[],tips:'请选择用户',prop:{name:'name',value:'id'},model:{icon:'hidden',label:{type:'text'}},clickClose:true});admin.get('backend/user/xm-select-data?id=0').then(function(data){userXmlSelect.update({data:data.tree,tree:{show:true,showFolderIcon:true,indent:15,strict:false,expandedKeys:[1],simple:true},autoRow:true});});};_proto.openEditor=function openEditor(id,title,data){var that=this;admin.openDialog('#'+id+'_editor',title,{area:['900px','600px'],btn:[_t('Save'),_t('Cancel')],yes:function yes(idx){that.saveMsg(id,idx);},btn2:function btn2(idx){this.cancel(idx);},cancel:function cancel(idx){layer.close(idx);that.savedData=form.val(id+'EditForm');}},function(){form.render(null,id+'EditForm');form.val(id+'EditForm',data||that.savedData);});};_proto.saveMsg=function saveMsg(id,idx){var that=this;if(!form.validate(id+'EditForm')){return;}admin.showLoading('#layui-layer'+idx,4,'0.65');var data=form.val(id+'EditForm');admin.post('backend/message/save/'+id,data).then(function(resp){admin.removeLoading('#layui-layer'+idx);if(resp&&resp.code===200){that.savedData={};that.dataTable.reloadData();layer.close(idx);}}).fail(function(resp){admin.removeLoading('#layui-layer'+idx);});};return TablePage;}();var page=new TablePage();page.init('#pageTable',pageData.table.cols,pageData.table.data);});{/literal}</script>